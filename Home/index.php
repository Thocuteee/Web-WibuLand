    ...
    <main>
        ...
        <div class="home-content">
            <div class="content">
                ...
                <div class="container">
                    <?php
                        // Khai báo kết nối DB để sử dụng bên ngoài hàm
                        global $conn;
                        
                        // === BƯỚC 1: LẤY DANH SÁCH YÊU THÍCH CỦA NGƯỜI DÙNG TỪ DB ===
                        ...
                        if ($user_id) {
                            ...
                            if ($stmt_wishlist) {
                                ...
                                while ($row = mysqli_fetch_assoc($result_wishlist)) {
                                    // Tạo key theo format: 'category_id'
                                    $key = $row['LoaiSanPham'] . '_' . $row['IdSanPham'];
                                    $user_wishlist[$key] = true; // Đặt giá trị true để check nhanh
                                }
                        ...
                        function getProducts($name_category, $conn, $sanpham,$link){
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    // === BƯỚC 2: KIỂM TRA VÀ TẠO ICON YÊU THÍCH ĐỘNG ===
                                    ...
                                    echo '<a href="' . $detail_url . '">'; // Thêm link cho tên sản phẩm
                                    echo '<div class="name">' . $row['Name'] . '</div>';
                                    echo '</a>';
                                    
                                    // Sửa lỗi logic tính Hết hàng: sử dụng SoLuongTonKho - SoLuongDaBan
                                    if ($row['SoLuongTonKho'] <= $row['SoLuongDaBan']) {
                                        echo '<div class="sold-out" style ="background: orange">Hết hàng</div>';
                                        
                                    } else {
                                        $Giacu = null; // Khởi tạo biến
                                        if ($row['Sale'] > 0) {
                                        ...
                                        // Giá hiển thị là giá đã sale (nếu Sale > 0)
