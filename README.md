...
| **Admin** | **CRUD Sản phẩm** | Trang quản trị đơn giản cho phép Xem, Thêm, Sửa, Xóa (CRUD) sản phẩm theo từng danh mục (`mohinh`, `magma`, `cosplay`). |
...
| `mohinh`, `magma`, `cosplay` | Lưu trữ dữ liệu sản phẩm theo danh mục. | `ID`, `Name`, `Gia`, `Sale`, `SoLuongTonKho`, `TheLoai`. |
